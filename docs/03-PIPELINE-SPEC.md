...
If, after admitting priority 1–4, no file could be resolved at all, or the resolved evidence carries no real content, the stage terminates the investigation as **`insufficient_context`** with an explanation. We do not guess. *(Revised at T4.4 from the original "fewer than 3 distinct files or fewer than 800 tokens" rule — no fixed count can separate a real, thin, self-contained bug from a correctly-handled external failure; see the implementation note below. Judging **fixability** from evidence volume is not this stage's job — §S6 below has its own `insufficient_context` exit for exactly that.)*
...
#### Implementation note — ranking, budget, quality scoring, and the threshold the corpus disproved (Phase 7 cleared)

Ranking, dedup, the 24,000-token budget, and quality scoring are built — `apps/worker/roottrace_worker/pipeline/retrieve/{ranking,quality,tokens,bundle}.py` — and implement this section's relevance formula and eviction table literally, including the two mechanisms this section only describes in prose: "sort by relevance and admit" plus a "separate" priority-ordered eviction pass collapse into one `(priority, -relevance)` sort with an identical result, and "trimmed to signatures only if tight" (priority 8) is implemented via an `ast`-based signature extraction, not just documented as an aspiration.

**No tokenizer dependency**, the same kind of decision as `ast` over Tree-sitter at T4.1–T4.3: `06` §2.2 routes every tier across two providers (Anthropic primary, OpenAI failover, reversed for the critic), and no single tokenizer is exact for both — Anthropic has no offline tokenizer at all. The estimate (`chars / 3.5`, rounded up) is deliberately biased to overcount rather than to be closest on average, since undercounting against a hard ceiling is the unrecoverable failure mode and overcounting is not.

**Budget is never exceeded across the corpus (0/25 over) and priority 1–2 items are never evicted (verified directly, not inferred) — the two bars T4.4 can hold on its own.**

**The original `insufficient_context` threshold — "fewer than 3 distinct files or fewer than 800 tokens" — could not be adjusted to a working number, because no fixed threshold exists.** Measuring the corpus against it literally: **18 of the 23 non-control cases** also terminated as `insufficient_context`, every one carrying `expected.final_status: "awaiting_decision"` (`14` §6.2). Hand-checked against several (`key-error-01`, `config-01`): retrieval was not missing anything real — `verify_signature` in `key-error-01` genuinely calls nothing but stdlib and defines no type worth expanding, so priority 1–4 mechanically tops out at one file. The threshold was calibrated for a retrieval richer than V1's deliberately narrow strategy set (P3: retrieve narrowly; 1 hop only; strategy C deferred to V2) naturally produces.

The natural fix — loosen the numbers — turned out to be impossible, not just untried: `external-03` (a real bug — `InventoryClient.reserve` has no circuit breaker) and `unfixable-01` (a control — the same function already handles the failure correctly) admit the *identical* 2 files and 1231 tokens of priority 1–4 evidence. `config-01` (also real) admits only 251. Every threshold that admits `config-01` also admits both controls; every threshold that rejects the controls also rejects `config-01`. **File/token volume cannot distinguish "a real, thin, self-contained bug" from "a correctly-handled external failure" — they are the same shape.**

**Decided by the coordinator:** judging fixability was never S5's question to answer — this section already gives S6 its own `insufficient_context` exit ("on evidence-binding failure … terminal `insufficient_context`", see §S6 below) for exactly the case where a model concludes "no defect, external cause." S5's threshold is lowered to what S5 can honestly judge: did retrieval resolve the failure point with any real content, or not (`MIN_ADMITTED_FILES = 1`, `MIN_ADMITTED_IN_APP_TOKENS = 1` in `ranking.py`). Under that bar, **all 25 corpus cases — the 23 fixable cases and both controls — correctly produce a `ContextBundle`**: retrieval genuinely succeeded for all 25, including the controls, whose `InventoryClient` code is real and retrievable, it simply contains no defect. Whether a case is fixable is not decided by S5 for any case, including the controls — that determination, and the controls' `expected.final_status: "insufficient_context"`, is a Phase 8 (S6) acceptance property now, not T4.4's. Corpus proof: `tests/integration/test_retrieve_ranking_corpus.py`.

See `docs/PROJECT-STATUS.md` for the full write-up.
