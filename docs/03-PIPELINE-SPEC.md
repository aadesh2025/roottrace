...
**Three of the criterion's four numbers are clean: budget is never exceeded across the corpus (0/25 over), priority 1–2 items are never evicted (verified directly, not inferred), and both controls (`unfixable-01`, `unfixable-02`) terminate as `insufficient_context` exactly as intended.**

**The fourth number is not clean, and this is a genuine finding, not a bug fixed by adjusting a threshold.** This section's own termination rule — *"if, after admitting priority 1–4, we still hold fewer than 3 distinct files or fewer than 800 tokens of `in_app` source, the stage terminates as `insufficient_context`"* — taken literally against what T4.3's four implemented strategies actually produce, also terminates **18 of the 23 non-control corpus cases**, every one of which carries `expected.final_status: "awaiting_decision"` (`14` §6.2) — ground truth that they are meant to reach reasoning, not abstain. Hand-checked against several (`key-error-01`, `config-01`): retrieval is not missing anything real. `verify_signature` in `key-error-01` genuinely calls nothing but stdlib and defines no type worth expanding — the bug is one dict index in one self-contained function, and priority 1–4 mechanically tops out at a single file. The threshold, read literally, was evidently calibrated for a retrieval richer than V1's deliberately narrow strategy set (P3: retrieve narrowly; 1 hop only; strategy C, which would routinely add a third or fourth file to thin cases, deferred to V2) naturally produces.

Left as found, undecided, per instruction: the threshold is not adjusted, and no code was tuned to make this corpus pass. See `docs/PROJECT-STATUS.md` for the full write-up and the decision this leaves open. **Phase 7 is not cleared to advance into Phase 8 while this stands.**
