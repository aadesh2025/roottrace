...
**Mechanism built at T5.3 — the corpus-wide accuracy bars above are verified at `T10.1` (Phase 15), not here.** `≥20/25 identify the correct root-cause file` and the full `unfixable-01`/`unfixable-02` vs. the other 23 split are both statistical claims about real model behaviour across the whole corpus; `15` already scopes that measurement to `T10.1`'s evaluation harness, and this ticket does not duplicate it. What is built and verified against `FakeProvider`: `RootCauseAnalysis` (`03` §S6's contract), evidence binding (`06` §4.2 — H1/H2/S6's half of H3, for real, not schema validation standing in for them), the correction-retry-once-then-`insufficient_context` ladder, and a `GatewayReasoner` composing all of it through the T5.1 gateway and T5.2 prompt system, tested end-to-end. One live-gated test (`test_reason_live.py`, skipped without a real key) runs `reason()` against 7 real fixture cases with a real model — a catastrophic-failure check, explicitly not the accuracy measurement. See `06` §6.4 and `PROJECT-STATUS.md` for the full accounting; whoever builds `T10.1` is the one who gets to say whether these two bars actually hold.
...
**Mechanism built at T5.4 — the corpus-wide `≥24/25 diffs apply cleanly` bar above is verified at `T10.1` (Phase 15), not here**, for the same reason `T5.3`'s `≥20/25` bar is deferred there: it is a statistical claim about real model output across the whole corpus, and `15` already scopes that measurement to `T10.1`'s evaluation harness. The other two accept items — zero forbidden-path touches, a regression test present whenever required — are deterministic properties of `validate.py`'s own enforcement, not of model accuracy, and **are** verified here against `FakeProvider` across every scope-violation and applicability-failure shape the constraint table names. What is built and verified: `Patch` (`03` §S7's contract), the in-memory `unidiff`-based diff-apply check (H5) against the actual retrieved window, the scope allowlist/forbidden-path/`must_not_modify`/test-deletion checks (H6), and a `GatewayPatcher` composing all of it through the T5.1 gateway and T5.2 prompt system, tested end-to-end. H4 (Tree-sitter symbol existence) is not built — `03` §S7's own constraint table has no row for it and no accept bar here requires it; disclosed, not silently skipped. One live-gated test (`test_patch_live.py`, skipped without a real key) runs the full S4→S5→S6→S7 chain against 5 real fixture cases with a real model — a catastrophic-failure check, explicitly not the accuracy measurement. See `06` §5.1 and `PROJECT-STATUS.md` for the full accounting; whoever builds `T10.1` is the one who gets to say whether the `≥24/25` bar actually holds.
...
**Built and verified against the real Docker daemon and the real `roottrace/sandbox-python:3.12` image — all four accept criteria above, individually, not a corpus-wide claim this time.** G0/G1 run host-side (`pipeline/validate/gates.py`); G2–G8 run inside the container (`apps/sandbox-runner/roottrace_sandbox_runner/gates.py`), dispatched fail-fast in `07` §6's own cheapest-first order. G4 — "the critical gate," per `03` §S8 — got the most scrutiny: a genuine fix with a genuinely reproducing regression test passes every gate (`test_a_genuine_fix_passes_every_gate`); a theatrical test that also passes on unpatched code is rejected (`test_g4_rejects_a_test_that_passes_on_unpatched_code`); a test that fails for an unrelated reason (a broken import, not the bug) is rejected too, not mistaken for reproducing anything (`test_g4_rejects_a_test_that_fails_for_an_unrelated_reason`) — all three confirmed with the actual `ValidationResult` printed, not just an assertion. G5 catches a "fix" that doesn't fix anything and routes the failure back through `03`'s own "diagnosis may be wrong" framing. G6's pre/post baseline correctly classifies `newly_failing` versus `already_failing`. G7 counts only new static findings (`new_high`/`new_medium`/`new_low`), HIGH-only gate-failing. G8 scans added lines only (reconstructed via `difflib`, since no raw diff travels into the container — `files_original`/`files_patched` do) and blocks a newly-introduced `eval()`. One scoping decision, disclosed rather than silently built in either extreme: G4/G5's exception-family check is enforced strictly only for the families that map cleanly onto specific built-in exceptions (`null_undefined`, `type_mismatch`, `key_index`, `resource`, `serialization`); the families a real codebase mostly defines its own exception classes for (`integration`, `data_db`, `auth`, `concurrency`, `unclassified`) are recorded, not gate-failing — a hardcoded builtin-only list for those would reject perfectly good regression tests on a technicality. See `PROJECT-STATUS.md` for the full accounting, including two `07` corrections found while building this (input/output delivery mechanism at T6.1, `existing_tests` needing content not just paths at T6.4).
...
**Built.** `CritiqueRequest` accepts only `diff: str`, never a `Patch` object — structurally, not just by convention, excluding S7's `explanation`/`risk_assessment`/`alternatives_considered`, which `03` §S10 explicitly says the critic must not see. `critique/v2.md`'s system layer *replaces* the shared L1 entirely (`A2` §6's own heading, "System layer override") rather than extending it, per the file's own pre-existing comment (written ahead of this ticket alongside T5.1/T5.2's prompt-registry work) describing how to split it on its `---` break; `split_critique_prompt` does exactly that, stripping the leading documentation comment first. `reasoning-b` tier per `06` §8.1. `blocking` and `security_review.clean` are recomputed by the stage, never trusted from the model's own self-report, even though the prompt asks the model to set both — both are facts already derivable from `verdict`/`findings`/`concerns`. No deterministic floor (there is no algorithmic substitute for an adversarial review) and not terminal either (`03` §S10: "proceed to S11 with `critic_component = 0`") — `CritiqueOutcome.unavailable` is S10's own third shape, distinct from S4's deterministic-floor pattern and S6/S7's terminal-on-failure pattern. Evidence citations on findings are deliberately not run through `reason.validate`'s literal-string binding check — `03` §S10 never asks for that rigor the way §S6 explicitly does, and the worked example's own evidence shape has no `excerpt` field to bind against; disclosed, not silently built anyway. Both accept criteria verified directly, including one full end-to-end test with a genuinely backdoored diff and a scripted reject verdict propagating through the real gateway into a blocking `Critique`.
